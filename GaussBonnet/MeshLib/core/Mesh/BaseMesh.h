...
*/
/*！
* \ brief CBaseMesh，所有类型的网格类的基类
*
*这是网格的基础类。 它包括一个顶点列表，
*边缘列表，面部列表。 所有几何对象都通过指针连接，
*顶点，边缘，面由半边连接。 网格类具有文件IO功能，
*支持.obj，.m和.off文件格式。 它提供欧拉运算符，每个运算符都是几何原型
*可以自由访问其邻居。
*
* \ tparam CVertex顶点类，派生自MeshLib :: CVertex类
* \ tparam CEdge边缘类，派生自MeshLib :: CEdge类
* \ tparam CFace face class，派生自MeshLib :: CFace类
* \ tparam CHalfEdge halfedge类，派生自MeshLib :: CHalfEdge类
*/
...
*/
	...
	*/
	//访问顶点 - id / *！ 通过其id \ param id访问顶点顶点id \返回顶点，其ID等于id。 NULL，如果没有这样的顶点。*/
	...
	*/
	//访问halfedge - halfedge键，顶点
	/*！
	通过其两个末端顶点访问halfedge
	\ param v0 halfedge的一个顶点
	\ param v1 halfedge的另一个顶点
	\返回连接v0和v1的halfedge，如果不存在这样的边，则返回NULL。
	*/
	...
	*/
	/*！
	通过目标顶点访问halfedge，并附加面。
	\ param v目标顶点
	\ param f附着脸
	\ return halfedge，其目标是v，附着面是f。 如果不存在这样的halfedge，则为NULL。
	*/
...
public:
	...
	*/
	...
	void read_vertices_m(const char * input);

	void writeHull_m(const char * output);
